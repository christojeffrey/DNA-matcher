    ...
    <>
      ...
      <Box p={24} pl={48} w="100%" position="relative" align="center">
        ...
        <FileUploader multiple={false} handleChange={handleChange} name="dnaSequence" types={fileTypes}>
          <Box w="60%" border="2px" borderColor="teal.dark" bg="main.500" rounded="lg" cursor="pointer" p="12" maxH="30vh" overflow={"auto"}>
            {file ? (
              <>
                <Center>
                  <Text>
        ...
        <Stack mt="12" w="60%">
          ...
          <Select bg="teal.dark" color="main.100" placeholder="Select disease" value={dnaCtx.Disease} onChange={handleDisease}>
            {dnaCtx.diseaseList != null &&
              dnaCtx.diseaseList.map((disease) => {
                return (
                  <option style={{ color: "black" }} value={disease}>
                    {disease}
                  </option>
                );
              })}
